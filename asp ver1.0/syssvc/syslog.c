 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: syslog.c 1176 2008-07-01 10:24:46Z ertl-hiro $
 */
...
static uint_t	syslog_lowmask;			/* 低レベル出力すべき重要度 */
...
syslog_wri_log(uint_t prio, const SYSLOG *p_syslog)
{
	 ...
	 *  ログ時刻の設定
	 */
	((SYSLOG *)(far) p_syslog)->logtim = _kernel_current_time;
	 ...
	 *  低レベル出力
	 */
	if ((syslog_lowmask & LOG_MASK(prio)) != 0U) {
...
syslog_ref_log(T_SYSLOG_RLOG *pk_rlog)
{
	...
	pk_rlog->lowmask = syslog_lowmask;
